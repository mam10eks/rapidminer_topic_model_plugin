...
/**
 * An abstract output port implementation. Only the {@link #deliver(com.rapidminer.operator.IOObject)} method is missing.
 * 
 * @author Nils Woehler
 *
 */
public abstract class AbstractOutputPort extends AbstractPort implements OutputPort {
	...
	public void connectTo(InputPort inputPort) throws PortException {
		...
		if (inputPort.getPorts().getOwner().getConnectionContext() != this.getPorts().getOwner().getConnectionContext()) {
			throw new PortException("Cannot connect " + getSpec() + " to " + inputPort.getSpec() + ": ports must be in the same subprocess, but are in "
