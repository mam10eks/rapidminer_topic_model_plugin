...
/**
 * Creates a panel which contains a {@link JTextField} and a {@link JLabel} which can be used to enter
 * the name of a repository entry. Invalid names (checked via {@link RepositoryLocation#isNameValid(String)})
 * are marked invalid while typing and an event is fired to signal invalid/valid entries.
 * <p>
 * To listen to the event (firing a simple {@link Boolean} to signal valid (<code>true</code>) and invalid (<code>false</code>) entries),
 * register your listener via {@link #addObserver(Observer, boolean)} or {@link #addObserverAsFirst(Observer, boolean)}.
 * 
 * @author Marco Boeck
 */
public class RepositoryEntryTextField extends JPanel implements Observable<Boolean>{
	...
	private void checkName() {
		...
		try {
			...
			LogService.getRoot().log(Level.SEVERE, I18N.getMessage(LogService.getRoot().getResourceBundle(), 
		...
		if (!valid) {
			...
			if (!empty) {
				entryErrorTextLabel.setText(I18N.getMessage(I18N.getGUIBundle(), "gui.dialog.repository_location.location_invalid_char.label", illegalSubstring));
			} else {
				entryErrorTextLabel.setText(I18N.getMessage(I18N.getGUIBundle(), "gui.dialog.repository_location.location_invalid_empty.label"));
			}
		} else {
