...
public class RangeAxisData {
	...
	/**
	 * @param rangeAxisConfig the rangeAxisConfig to set
	 */
	...
	public List<PlotConfigurationError> getWarnings() {
		...
		//		} 	
			...
			if ((DataStructureUtils.greaterOrAlmostEqual(lowerViewBound, maxYValue, 1E-6) || DataStructureUtils.greaterOrAlmostEqual(minYValue, upperViewBound, 1E-6))
					&& !equalMinAndMaxValue) {
				warnings.add(new PlotConfigurationError("user_range_includes_no_data", rangeAxisConfig.getLabel()));
			}
		}
	...
	public List<PlotConfigurationError> getErrors() {
		...
		if (rangeAxisConfig.getValueType() == ValueType.DATE_TIME && (Double.isInfinite(lowerViewBound) || Double.isInfinite(upperViewBound))) {
			errors.add(new PlotConfigurationError("infite_range_for_date_axis", rangeAxisConfig.getLabel()));
			return errors;
		}
		
		if (rangeAxisConfig.isUsingUserDefinedUpperViewBound() && DataStructureUtils.greaterOrAlmostEqual(lowerViewBound, upperViewBound, 1E-6)) {
			errors.add(new PlotConfigurationError("axis_upper_range_below_lower_auto_range", rangeAxisConfig.getLabel(), lowerViewBound + ""));
			return errors;
		}
