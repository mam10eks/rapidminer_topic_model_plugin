		...
		for (int binIdx = 1; binIdx <= binCount; ++binIdx) {
			...
			for (int dvIdx = currentUpperIdx + 1; dvIdx < distinctValueCount; ++dvIdx) {
				...
				if (nextValueCount >= aimedValueCountForCurrentBin || !enoughRemainingDistinctValues) {
					...
					if ((currentDifferenceFromAverage < nextDifferenceFromAverage || !enoughRemainingDistinctValues) && valueCountInBin > 0) {
			...
			NumericalValueRange currentGroup = new NumericalValueRange(lowerBound, upperBound, columnIdx, null, true, binIdx == binCount);
