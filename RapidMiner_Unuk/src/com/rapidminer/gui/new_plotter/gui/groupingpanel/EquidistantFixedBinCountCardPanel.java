...
public class EquidistantFixedBinCountCardPanel extends AbstractGroupingCardPanel {
	...
	protected void upperBoundChanged() {
		...
		if (dimensionConfig != null) {
			final EquidistantFixedBinCountBinning grouping = (EquidistantFixedBinCountBinning) dimensionConfig.getGrouping();
			if (grouping != null) {
				final double oldUpperBound = (Double) grouping.getMaxValue();
				...
				if (oldUpperBound != newUpperBound) {
					...
					if (DataStructureUtils.almostEqual(currentLowerBound, newUpperBound, 1E-6) || newUpperBound <= currentLowerBound) {
	...
	protected void lowerBoundChanged() {
		...
		if (dimensionConfig != null) {
			final EquidistantFixedBinCountBinning grouping = (EquidistantFixedBinCountBinning) dimensionConfig.getGrouping();
			if (grouping != null) {
				final double oldLowerBound = (Double) grouping.getMinValue();
				...
				if (oldLowerBound != newLowerBound) {
					...
					if (DataStructureUtils.almostEqual(currentUpperBound, newLowerBound, 1E-6) || currentUpperBound <= newLowerBound) {
