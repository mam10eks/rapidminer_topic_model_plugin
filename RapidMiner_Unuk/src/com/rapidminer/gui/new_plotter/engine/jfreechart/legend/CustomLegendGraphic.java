...
/**
 * A {@link LegendGraphic} which correctly translates gradients,
 * such that they begin at the correct position instead of the left screen edge.
 * 
 * @author Marius Helf
 *
 */
	...
	public void draw(Graphics2D g2, Rectangle2D area) {
        ...
        if (isLineVisible()) {
            ...
            Shape aLine = ShapeUtilities.createTranslatedShape(getLine(),
                    getShapeAnchor(), location.getX(), location.getY());
        ...
        if (isShapeVisible()) {
            ...
            Shape s = ShapeUtilities.createTranslatedShape(getShape(),
                    getShapeAnchor(), location.getX(), location.getY());
            if (isShapeFilled()) {
                ...
                if (p instanceof GradientPaint) {
                	...
                	p = getTranslatedLinearGradientPaint(gradient, new Point2D.Double(bounds.getMinX(), bounds.getMinY()), new Point2D.Double(bounds.getMaxX(), bounds.getMaxY()), false);
                }
                g2.setPaint(p);
                g2.fill(s);
            }
