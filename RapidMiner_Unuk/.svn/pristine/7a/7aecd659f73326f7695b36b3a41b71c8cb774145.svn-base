...
public class DimensionConfigPanel extends AbstractConfigurationPanel {
	...
	private void createComponents() {
		...
		// setup date format configuration components
		{
			...
			useDefaultDateFormatCheckBox = new JCheckBox(I18N.getGUILabel("plotter.configuration_dialog.use_default_date_format.label"));
		...
		// add grouping combo box
		{
				...
				public void popupMenuWillBecomeInvisible(PopupMenuEvent e) {
					...
					if (dimensionConfig != null) {
						...
						if ((newGroupingNoneAndOldGroupingSet || newGroupingAndNotGrouping || groupingTypeChanged)) {
							...
							try {
								newGrouping = ValueGroupingFactory.getValueGrouping(groupingType, dimensionConfig.getDataTableColumn(), categorical, dimensionConfig.getDateFormat());
			...
			groupingConfigButton = new JToggleButton(new PopupAction(true, "plotter.configuration_dialog.configure_grouping", groupingConfigurationPanel,
		...
		//		{
		//
		//			JTable valueSourceGroupingSelectionTable = new JTable(new GroupingSelectionTableModel(getPlotConfiguration()));
				...
				public void stateChanged(ChangeEvent e) {
					...
					if (dimensionConfig != null) {
						...
						if (oldUpperBound != newUpperBound) {
							//							Double currentLowerBound = dimensionConfig.getUserDefinedLowerBound();
							//							if (oldUpperBound != null && currentLowerBound != null) {
							//								if (Math.abs(newUpperBound - currentLowerBound) < 1E-5 || newUpperBound <= currentLowerBound) {
	...
	protected void adaptGUI() {
		...
		if (dimensionConfig != null && dimensionConfig.getDataTableColumn().isValidForDataTable(dataTable)) {
			...
			boolean enableOptions = !dimensionConfig.getDataTableColumn().isNominal() && !dimensionConfig.getDataTableColumn().isDate();
...
//				boolean groupingVisible = dimensionConfig.getDataTableColumn().getValueType() != ValueType.NOMINAL;
...
//				groupingConfigButton.setVisible(groupingVisible);
			...
			DefaultComboBoxModel model = new DefaultComboBoxModel(validGroupings);
			...
			boolean nominal = (dimensionConfig.getDataTableColumn().getValueType() == ValueType.NOMINAL) && dimensionConfig.isNominal();
