 ...
 * 
 * All groupings inheriting this class have in common that they depend on exactly
...
public abstract class AbstractValueGrouping implements ValueGrouping {
	 ...
	 * The default implementation invalidates the cache.
	 */
...
//	}
	...
	/**
	 * Configures the value ranges in valueGroups such that their toString() method
	 * display the optimal number of digits to discriminate both the intervals itself
	 * (upper/lower bound) and also discriminate intervals from their neighbours.
	 * For each data point a different precision might be calculated.
	 * 
	 * For dates this functions searches for the biggest displayed time unit by looking
	 * at the distance between the two farthest data points, and for the smallest displayed
	 * value by looking at the closest distance between two neighbouring data points.
	 * For all date values the same format string is used.
	 * 
		...
		if (valuesAreDates) {
			...
			for (ValueRange valueGroup : valueGroups ) {
				...
				if (previous != null) {
					...
					int precisionUpper = Math.min(DataStructureUtils.getOptimalPrecision(current.getLowerBound(), current.getUpperBound()), DataStructureUtils.getOptimalPrecision(current.getUpperBound(), next.getUpperBound()));
					...
					int precisionLower = DataStructureUtils.getOptimalPrecision(current.getLowerBound(), current.getUpperBound());
					int precisionUpper = Math.min(DataStructureUtils.getOptimalPrecision(current.getLowerBound(), current.getUpperBound()), DataStructureUtils.getOptimalPrecision(current.getUpperBound(), next.getUpperBound()));
					if (precisionUpper >= Integer.MAX_VALUE) {
						precisionUpper = precisionLower;
					}
