...
/**
 * A cell editor for date parameters. It renders a {@TextField} and {@link DatePicker} to choose dates comfortable.
 * 
 * @author Nils Woehler
 *
 */
public class DateValueCellEditor extends AbstractCellEditor implements PropertyValueCellEditor {
		...
		datePicker.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!updatingComponents.get()) {
					...
					if (date != null) {
						textField.setText(ParameterTypeDate.DATE_FORMAT.format(date));
						fireEditingStopped();
					}
				}
			}
	...
	private void updateComponents(Object value) {
		...
		Date parsedDate;
		try {
			parsedDate = ParameterTypeDate.DATE_FORMAT.parse(textField.getText());
