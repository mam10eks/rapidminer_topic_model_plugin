	...
	private void createComponents(JTree plotConfigTree, DataTableColumnListTransferHandler th) {
		...
		// create renaming row
		{
...

			{
				...
				// add checkbox
				{
					automateLabelCheckBox = new JCheckBox(I18N.getGUILabel("plotter.configuration_dialog.automatic_axis_label.label"));
		...
		// add format button
		{
			...
			formatConfigureButton = new JToggleButton(new PopupAction(true, "plotter.configuration_dialog.format_button", seriesTypeConfigurationPanel,
		...
		// add aggregation function
		{
			...
			JPanel aggragationPanel = new JPanel(new GridBagLayout());
			{
				...
				aggregationComboBox.addPopupMenuListener(new PopupMenuListener() {
					...
					public void popupMenuWillBecomeInvisible(PopupMenuEvent e) {
						...
						if (selectedValueSource != null) {
							...
							boolean process = getPlotConfiguration().isProcessingEvents();  // save old state
							getPlotConfiguration().setProcessEvents(false); // set process events false
							...
							getPlotConfiguration().setProcessEvents(process); // restore old processing state
						}
					}
				...
				aggregationComboBox.setSelectedItem(AbstractAggregationFunction.KNOWN_AGGREGATION_FUNCTION_NAMES[AbstractAggregationFunction.COUNT]);
				...
				final String content = I18N.getGUILabel("plotter.configuration_dialog.value_source_panel.show_grouping_configuration.label");
		...
		// add legend popup button
		{
			...
			windowingButton = new JToggleButton(new PopupAction(true, "plotter.configuration_dialog.configure_windowing", windowConfigurationPanel,
					PopupAction.PopupPosition.HORIZONTAL));
		...
		// add first error attribute
		{
			...
			JPanel firstErrorAttributePanel = new JPanel(new GridBagLayout());
			{
				...
				firstUtilityRemoveAttributeButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.remove_button") {
			...
			// create aggregation combobox
			{
				...
				firstUtilityAggregationComboBox.addPopupMenuListener(new PopupMenuListener() {
					...
					public void popupMenuWillBecomeInvisible(PopupMenuEvent e) {
						ValueSource selectedValueSource = getSelectedValueSource();
						if (selectedValueSource != null) {
							selectedValueSource.setAggregationFunction(SeriesUsageType.INDICATOR_1, (AggregationFunctionType) firstUtilityAggregationComboBox.getSelectedItem());
						}
					}
		...
		// add second error attribute
		{
			...
			JPanel secondErrorAttributePanel = new JPanel(new GridBagLayout());
			{
				...
				secondUtilityRemoveAttributeButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.remove_button") {
			...
			//			addThreeComponentRow(this, secondUtilityAttributeLabel, secondErrorAttributePanel, secondUtilityAggregationComboBox);
		}
