...
public class DefaultDimensionConfig extends AbstractDimensionConfig implements ValueGroupingListener, ValueRangeListener {
	...
	public DefaultDimensionConfig(PlotConfiguration plotConfiguration, DataTableColumn dataTableColumn, PlotDimension dimension) {
	...
	public void setGrouping(ValueGrouping valueGrouping) {
		...
		if (!isGrouping() || (valueGrouping != null && valueGrouping.getGroupingType() != getGrouping().getGroupingType()) || valueGrouping != this.valueGrouping) {
			...
			fireDimensionConfigChanged(new DimensionConfigChangeEvent(this, getDimension(), DimensionConfigChangeType.ABOUT_TO_CHANGE_GROUPING));
	...
	public void setUserDefinedRange(NumericalValueRange range) {
		...
		if (this.userDefinedRange != range) {
			...
			fireDimensionConfigChanged(new DimensionConfigChangeEvent(this, getDimension(), new ValueRangeChangeEvent(userDefinedRange, ValueRangeChangeType.RESET)));
		}
	}
	...
	public List<PlotConfigurationError> getErrors() {
		...
		if (!getSupportedValueTypes().contains(valueType)) {
			PlotConfigurationError error = new PlotConfigurationError("illegal_dimension_type", getDimension().getName(), valueType);
			errorList.add(error);
			return errorList;
		}
		...
		if (isUsingUserDefinedLowerBound() && isUsingUserDefinedUpperBound()) {
			if (DataStructureUtils.greaterOrAlmostEqual(getUserDefinedLowerBound(), getUserDefinedUpperBound(), 1E-6)) {
				PlotConfigurationError error = new PlotConfigurationError("min_filter_greater_max_filter", getDimension().getName());
				errorList.add(error);
				return errorList;
			}
		}
	...
	public List<PlotConfigurationError> getWarnings() {
		...
		if (isLogarithmic() && !isNumerical()) {
			if (!isNominal()) {
				// ignore nominal axes, since there logarithmic cannot be set and is not visible in the gui
	...
	public void setLogarithmic(boolean logarithmic) {
		if (logarithmic != this.logarithmicDimensionAxis) {
			...
			fireDimensionConfigChanged(new DimensionConfigChangeEvent(this, getDimension(), logarithmic, DimensionConfigChangeType.SCALING));
		}
	}
	...
	public void setAutoNaming(boolean autoNaming) {
		if (autoNaming != this.autoNaming) {
			...
			setAutoLabelIfEnabled();
			fireDimensionConfigChanged(new DimensionConfigChangeEvent(this, getDimension(), autoNaming, DimensionConfigChangeType.AUTO_NAMING));
		}
	}
	...
	public void colorSchemeChanged() {
		fireDimensionConfigChanged(new DimensionConfigChangeEvent(this, getDimension(), DimensionConfigChangeType.COLOR_SCHEME));
	}

	@Override
	public String toString() {
		return "DefaultDimensionConfig for " + getDimension().getName() + " (" + (isGrouping() ? "grouping" : "not grouping") + ")";
	}
	...
	public void setUserDefinedDateFormatString(String formatString) {
		if (formatString != null ? !formatString.equals(this.userDefinedDateFormatString) : userDefinedDateFormatString != null) {
