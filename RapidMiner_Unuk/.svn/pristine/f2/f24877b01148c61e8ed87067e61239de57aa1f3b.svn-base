 ...
 * 
 * Each ValueSource holds a reference to a DimensionConfig which configures the domain for
 * this value source. Since all value sources share the same domain axis but hold individual
 * domain configurations, there are some constraints for these configurations. 
 * Because of that the contract for the domain configuration is that all operations which change
 * the value type or the categories of the domain must be passed through the {@link DomainConfigManager}
 * of the the {@link PlotConfiguration} and may not be called directly on the object returned
 * by getDomainConfig().
 * 
...
public class ValueSource implements AggregationWindowingListener, SeriesFormatListener {
	 ...
	 */
	public ValueSource(PlotConfiguration plotConfiguration, DataTableColumn mainDataTableColumn, AggregationFunctionType aggregationFunctionType, boolean grouped) {
	 ...
	 */
	private ValueSource(DomainConfigManager domainConfigManager, DataTableColumn mainDataTableColumn, AggregationFunctionType aggregationFunctionType, boolean grouped, int Id) {
	...
	/**
	 * Returns the {@link ValueType} of the output values of this value source (which are not the necessarily the input values). 
	 */
	...
	/**
	 * Returns true if the output values of this value source (not the necessarily the input values) are
	 * numerical, either because the input data itself is numerical, or the applied grouping and aggregation
	 * function results in numerical values.  
	 */
	...
	//			return false; // use value from DimensionConfig, e.g. different color for each item from ColorProvider
	//		}
	//		if (dimensionConfig != null && dimensionConfig.isGrouping()) {
	//			return true; // use value from DimensionConfig for this series
	//		}
	//
	//		throw new RuntimeException("Should not happen, one of the above conditions should always be true.");
	//	}
	...
	public boolean useSeriesFormatForDimension(PlotConfiguration plotConfig, PlotDimension dimension) {
		...
		if (!dimensionConfig.isGrouping() && isUsingDomainGrouping()) {
			...
			// --> use value from SeriesFormat (not possible to use e.g. color from ColorProvider for each item, 
			//     because each item is aggregated from possibly many items with possibly different values for 
			//     the given dimension) 
			return true;
		}
		return false;
	...
	public void setUseRelativeUtilities(boolean relativeUtilities) {
		if (relativeUtilities != this.useRelativeUtilities) {
			...
			fireValueSourceChanged(new ValueSourceChangeEvent(this, ValueSourceChangeType.USE_RELATIVE_UTILITIES, relativeUtilities));
		}
	}
		...
		if (useDomainGrouping) {
			for (SeriesUsageType usageType : getDefinedUsageTypes()) {
				...
				if (!doesAggregationFunctionSupportValueType(aggregationFunction, valueType)) {
					errors.add(new PlotConfigurationError("value_type_not_supported_by_aggregation", this.toString(), usageType, valueType, aggregationFunction.getName()));
				}
			}
		}
	...
	/**
	 * Exchanges the domainConfigManager of this ValueSource. Should only be called directly after cloning.
	 */
