 ...
 * {@link PlotConfiguration}. This manager is necessary, because switching
...
public class DomainConfigManager extends AbstractDimensionConfig implements DimensionConfigListener, Cloneable, AxisParallelLinesConfigurationListener {
	...
	public DomainConfigManager(PlotConfiguration plotConfiguration, DataTableColumn domainColumn) {
		...
		try {
			groupedMasterDimensionConfig.setGrouping(new EquidistantFixedBinCountBinning(5, Double.NaN, Double.NaN, domainColumn, domainColumn.isNominal(), getDateFormat()));
	...
	public void setDataTableColumn(DataTableColumn column) {
		...
		if(column.isNominal() && groupedMasterDimensionConfig.getGrouping().getGroupingType() != GroupingType.DISTINCT_VALUES) {
			setFireEvents(false);
			try {
				setGrouping(ValueGroupingFactory.getValueGrouping(GroupingType.DISTINCT_VALUES, getDataTableColumn(), true, getDateFormat()));
			} catch (ChartConfigurationException e) {
...
//			PlotConfigurationError error = new PlotConfigurationError("illegal_domain_type", getValueType(), ValueType.INVALID);
//			errors.add(error);
//			return errors;
//		}
	...
	public void dimensionConfigChanged(DimensionConfigChangeEvent change) {
		...
		switch(type) {
			...
			case GROUPING_CHANGED:
				domainManagerEvent = new DimensionConfigChangeEvent(this, change.getDimension(), change.getGroupingChangeEvent());
				break;
			case LABEL:
				domainManagerEvent = new DimensionConfigChangeEvent(this, change.getDimension(), change.getLabel());
				break;
			case RANGE:
				domainManagerEvent = new DimensionConfigChangeEvent(this, change.getDimension(), change.getValueRangeChangedEvent());
				break;
			case RESET:
				domainManagerEvent = new DimensionConfigChangeEvent(this, change.getDimension(), type);
				break;
