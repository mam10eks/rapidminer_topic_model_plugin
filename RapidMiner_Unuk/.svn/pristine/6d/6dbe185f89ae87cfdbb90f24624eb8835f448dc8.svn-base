 ...
 * 
 * Currently only zip files are supported, but the operator could be extended
 * to support also other types of archives like rar, tar, ...
 * 
 * @author Marius Helf
 *
 */
public class CreateArchiveFileOperator extends Operator {
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(new ParameterTypeCategory(PARAMETER_BUFFER_TYPE, "Defines where the buffer for the archive file will be created. A memory buffered archive file will usually perform faster in terms of execution time, but the complete archive file must be kept in memory, which can lead to problems if large files or a large amount of files is added to the archive.", BUFFER_TYPES, BUFFER_TYPE_FILE, true));
		types.add(new ParameterTypeBoolean(PARAMETER_USE_DEFAULT_COMPRESSION_LEVEL, "This parameter allows you to override the default compression level. The default compression level depends on the host machine, but usually offers a reasonable trade-off between execution time and compression factor.", true, true));
		ParameterType type = new ParameterTypeCategory(PARAMETER_COMPRESSION_LEVEL, "The default compression level of the created zip file is specified by this parameter. This level may be overridden in the subsequent Add Entry to Zip File operators on a per-entry base. A compression level of 0 stands for no compression, whereas the highest level 9 means best compression. In general, higher compression levels result also in a higher runtime.", COMPRESSION_LEVELS, COMPRESSION_LEVEL_DEFAULT, true);
