...
public class PlotConfigurationTreeModel extends DefaultTreeModel implements PlotConfigurationListener {
	...
	private void valueSourceAdded(int index, ValueSource valueSource, RangeAxisConfig source) {
		...
		if (rangeAxisNode != null) {
			...
			// add new tree node
			insertNodeInto(newChild, (MutableTreeNode) rangeAxisNode, index);
	...
	private void fillNewPlotConfigNode(PlotConfiguration plotConfig) {
		...
		//CAUTION: If you add more dimension configs you have to adapt NUMBER_OF_PERMANENT_DIMENSIONS!!!

		// add domain dimension config
		insertNodeInto(new DimensionConfigTreeNode(PlotDimension.DOMAIN, plotConfig.getDomainConfigManager()), (MutableTreeNode) root, root.getChildCount());

		// add color dimension config
		insertNodeInto(new DimensionConfigTreeNode(PlotDimension.COLOR, plotConfig.getDimensionConfig(PlotDimension.COLOR)), (MutableTreeNode) root, root.getChildCount());

		// add shape dimension config
		insertNodeInto(new DimensionConfigTreeNode(PlotDimension.SHAPE, plotConfig.getDimensionConfig(PlotDimension.SHAPE)), (MutableTreeNode) root, root.getChildCount());

		// add size dimension config
		insertNodeInto(new DimensionConfigTreeNode(PlotDimension.SIZE, plotConfig.getDimensionConfig(PlotDimension.SIZE)), (MutableTreeNode) root, root.getChildCount());
