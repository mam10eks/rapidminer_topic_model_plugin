		...
		if (destEntry instanceof Folder) {
			...
			if (containsEntry) {
				if (overwrite) {
					...
					try {
						existingDestRepoLoc = new RepositoryLocation(destinationFolder.getLocation(), sourceRepoLoc.getName());
					...
					throw new UserError(this, "repository_management.relocate_repository_entry", destRepoLoc, "Entry already exsists but overwriting is disabled.");
				}
			}
