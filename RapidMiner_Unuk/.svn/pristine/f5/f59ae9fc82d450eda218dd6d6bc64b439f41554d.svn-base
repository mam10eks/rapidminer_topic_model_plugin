...
public class ValueSourceData {
	 ...
	 */
	private void createGroupCellData(Vector<PlotDimension> dimensionList, int dimensionIdx, DataTable data, GroupCellKey groupCellKey) {
		if (data == null) {
			return;
		}
		...
		if (dimension == PlotDimension.DOMAIN) {
			...
			dimensionConfig = (DefaultDimensionConfig) plotInstance.getCurrentPlotConfigurationClone().getDimensionConfig(dimension);
		}
		...
		if (allValueGroups == null) {
			...
			// apply AggregationWindowing on DOMAIN dimension and store it in a vector for fast access by index
			Vector<ValueRange> valueGroupsForFiltering;
			if (dimension == PlotDimension.DOMAIN) {
				valueGroupsForFiltering = new Vector<ValueRange>(valueSource.getAggregationWindowing().applyOnGrouping(allValueGroups));
			} else {
				valueGroupsForFiltering = new Vector<ValueRange>(allValueGroups);
			}

			// recurse, or store data for group if we are in the last dimension of the dimension list
			int idx = 0;
	...
	private void applyGrouping() {
		...
		createGroupCellData(dimensionList, 0, plotInstance.getPlotData().getDataTable(valueSource.isSamplingSuggested()), null);
	}
	 ...
	 */
	private GroupCellSeriesData recursivelyGetSeriesDataForAllGroupCells(Vector<PlotDimension> dimensions, int dimensionIdx, GroupCellKey groupCellKey,
			GroupCellSeriesData dataForAllGroupCells) {
		if (groupCellKey == null) {
			groupCellKey = new GroupCellKey();
		}
		...
		if (dimensionIdx < dimensions.size()) {
			...
			DefaultDimensionConfig currentDimensionConfig = (DefaultDimensionConfig) plotInstance.getCurrentPlotConfigurationClone().getDimensionConfig(currentDimension);
			DimensionConfigData currentDimensionConfigData = plotInstance.getPlotData().getDimensionConfigData(currentDimensionConfig);
			...
			if (xValueGroups == null) {
					...
					for (DataTableRow row : dataForCurrentCell) {
						for (SeriesUsageType usageType : valueSource.getDefinedUsageTypes()) {
							...
							for (PlotDimension dimension : dimensions) {
								DimensionConfig dimensionConfig = plotInstance.getCurrentPlotConfigurationClone().getDimensionConfig(dimension);
				...
				//x-axis is grouping --> iterate over all groups on x-axis and get aggregated values to generate series
	...
	private void updateDataTableColumns() {
		...
		for (SeriesUsageType seriesUsageType : valueSource.getDefinedUsageTypes()) {
			...
			int columnIdx = DataTableColumn.getColumnIndex(plotInstance.getPlotData().getOriginalDataTable(), dataTableColumn);
	...
	public String getStringForValue(SeriesUsageType seriesUsage, double y) {
		if (valueSource.isNominal()) {
			return plotInstance.getPlotData().getValueMappingDataTable().mapIndex(getDataTableColumnIdx(seriesUsage), (int) y);
		} else {
			return null;
		}
	}
