...
public class DataTableColumnDropTextFieldTransferHandler extends AbstractPatchedTransferHandler implements TreeSelectionListener {
	...
	public DataTableColumnDropTextFieldTransferHandler(JTree plotConfigTree, SeriesUsageType seriesUsage, AttributeDropTextField aDTF) {
	...
	public boolean importData(TransferSupport support) {
		...
		try {
			if (currentValueSource != null) {
				...
				currentValueSource.setDataTableColumn(seriesUsageType, dataTableColumnCollection.getDataTableColumns().get(0));
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		if (lastPathComponent instanceof ValueSourceTreeNode) {
			...
			currentValueSource = (ValueSource) selectedValueSource;
