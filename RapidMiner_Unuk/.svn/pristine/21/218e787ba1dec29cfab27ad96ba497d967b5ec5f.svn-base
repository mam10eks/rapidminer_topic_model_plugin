 ...
 * 
 */
public class ChartConfigurationPanel extends AbstractConfigurationPanel implements MasterOfDesasterListener, DragListener, PlotConfigurationProcessingListener {
	...
	private void createPopups() {
		...
		// creat popup menus
		{
			...
			removeRangeAxisMenuItem = new JMenuItem(I18N.getGUILabel("plotter.configuration_dialog.remove_axis_menu_item.label"));
			...
			JMenuItem menuItem = new JMenuItem(I18N.getGUILabel("plotter.configuration_dialog.remove_source_menu_item.label"));
	...
	private void createComponents() {
		...
		JButton showConfigButton = new JButton(new ResourceAction(smallIcons, "plotter.configuration_dialog.show_configuration") {
	...
	private JPanel createRightSidePanel() {
		...
		// add chart panel to box
		{
			...
			chartPanelScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			chartPanelScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
		...
		// add status text field
		{
			...
			statusTextAreaScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private JPanel createLeftSidePanel() {
		...
		// add panel with reset and hide buttons
		{
			...
			{
				resetButton = new JButton(new ResourceAction(smallIcons, "plotter.configuration_dialog.reset_configuration") {

					private static final long serialVersionUID = 1L;

					@Override
					public void actionPerformed(ActionEvent e) {
						if (ConfirmDialog.showConfirmDialogWithOptionalCheckbox("plotter.configuration_dialog.reset_configuration", ConfirmDialog.YES_NO_OPTION, null,
								ConfirmDialog.NO_OPTION, false) == ConfirmDialog.YES_OPTION) {
			...
			{
				JButton hideButton = new JButton(new ResourceAction(smallIcons, "plotter.configuration_dialog.hide_configuration") {
		...
		// add attribute list
		{
			...
			attributeListScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
			attributeListScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
		...
		// add plot configuration tree
		{
			...
			// create tree
			plotConfigurationTree = new PlotConfigurationTree(getPlotConfiguration(), dataTable, attributeListTransferHandler);
	...
	private void removeRangeAxisAction() {
		RangeAxisConfigTreeNode node = (RangeAxisConfigTreeNode) plotConfigurationTree.getSelectionPath().getLastPathComponent();

		RangeAxisConfig axisConfig = (RangeAxisConfig) node.getUserObject();
	...
	private void removeValueSourceAction() {
		...
		ValueSource source = (ValueSource) node.getUserObject();
		...
		if (parent == null) {
			...
			RangeAxisConfig rangeAxis = (RangeAxisConfig) parent.getUserObject();
			rangeAxis.removeValueSource(source);
		}
	}

	private void removeAttributeFromDimensionAction() {
		DimensionConfigTreeNode node = (DimensionConfigTreeNode) plotConfigurationTree.getSelectionPath().getLastPathComponent();
		DimensionConfig config = (DimensionConfig) node.getUserObject();
		if (config != null) {
			if (config.getDimension() != PlotDimension.DOMAIN) {
				getPlotConfiguration().setDimensionConfig(config.getDimension(), null);
			} else {
				getPlotConfiguration().getDomainConfigManager().setDataTableColumn(new DataTableColumn(null, ValueType.INVALID));
			}
		}
	}
