...
public class ColorSchemeDialog extends ButtonDialog implements PlotConfigurationListener {
	...
	private DefaultListModel nominalColorListModel;
	private DefaultComboBoxModel gradientStartColorComboBoxModel;
	private DefaultComboBoxModel gradientEndColorComboBoxModel;
	private DefaultComboBoxModel colorSchemeComboBoxModel;

	private JComboBox gradientEndColorComboBox;
	private JComboBox gradientStartColorComboBox;
	...
	public ColorSchemeDialog(Component actionComp, String key, PlotConfiguration plotConfig, Object... arguments) {
		super(key, true, arguments);
		this.plotConfig = plotConfig;

		initializing = true;

		nominalColorListModel = new DefaultListModel();
		gradientStartColorComboBoxModel = new DefaultComboBoxModel();
		gradientEndColorComboBoxModel = new DefaultComboBoxModel();
	...
	private void createComponents() {
		...
		// creat popup menus
		{
			...
			removeMenuItem = new JMenuItem(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.remove_color_menu_item.label"));
			...
			changeColorMenuItem = new JMenuItem(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.change_color_menu_item.label"));
			...
			moveUpColorMenuItem = new JMenuItem(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.move_up_menu_item.label"));
			...
			moveDownColorMenuItem = new JMenuItem(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.move_down_menu_item.label"));
		...
		// create containing panel
		{

			{
				...
				configurePanel.setBorder(BorderFactory.createTitledBorder(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.scheme_configuration_border.label")));
	...
	private JPanel createEndGradientPanel() {
		...
		{
			gradientEndColorComboBox = new JComboBox(gradientEndColorComboBoxModel);
	...
	private JPanel createStartGradientPanel() {
		...
		{
			gradientStartColorComboBox = new JComboBox(gradientStartColorComboBoxModel);
	...
	private JPanel createColorCategoriesPanel() {
		...
		// add button panel
		{
			...
			{
				removeCategoryColorButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.color_scheme_dialog.remove_category_color_button") {
			...
			{
				addCategoryButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.color_scheme_dialog.add_category_color_button") {
...

		{
			...
			// add list of categorie colors
			{
				...
				colorList = new JList(nominalColorListModel);
				...
				colorList.addKeyListener(new KeyListener() {
					...
					public void keyPressed(KeyEvent e) {
						...
						if (key == KeyEvent.VK_DOWN && e.isControlDown()) {
							moveSelectedColorDownAction();
						}
					}
				});
				...
				colorListScrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
				colorListScrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
			...
			// add up/down button panel
			{
				...
				// add down button
				{
					JButton downButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.move_color_down") {
	...
	private JPanel createSchemeComboBoxPanel() {
		...
		// create categories configuration panel
		{
			...
			// add button panel
			{
				...
				{
					renameSchemeButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.color_scheme_dialog.rename_scheme_button") {
				...
				// remove scheme button
				{
					removeSchemeButton = new JButton(new ResourceAction(true, "plotter.configuration_dialog.color_scheme_dialog.remove_scheme_button") {

						private static final long serialVersionUID = 1L;

						@Override
						public void actionPerformed(ActionEvent e) {
							ConfirmDialog dialog = new ConfirmDialog("plotter.configuration_dialog.confirm_color_scheme_delete", ConfirmDialog.YES_NO_OPTION, false);
	...
	private void createPreviewPlotBackend(Color background, int groupinBins) {
		...
		gradientPlotConfig.setTitleText(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.numerical_gradient_preview.label"));
		...
		DefaultDimensionConfig colorDimension = new DefaultDimensionConfig(gradientPlotConfig, mainColumn, PlotDimension.COLOR);
		...
		nominalPlotConfig.setTitleText(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.nominal_color_preview.label"));
		...
		DefaultDimensionConfig nominalColorDimension = new DefaultDimensionConfig(nominalPlotConfig, mainColumn, PlotDimension.COLOR);
	...
	private Color createColorDialog(Color oldColor) {
		return JColorChooser.showDialog(this, I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.new_category_color.label"), oldColor);
	}
	...
	private void moveSelectedColorUpAction() {
		...
		if (color != null) {
			...
			final int oldIndex = nominalColorListModel.indexOf(color);
			...
			getCurrentActiveColorScheme().addColor(newIndex, ColorRGB.convertColorToColorRGB(color));

			adaptNominalPlot();

			colorList.setSelectedValue(color, true);
		}
	}
	...
	private void replaceSelectedColorAction() {
		Color oldColor = (Color) colorList.getSelectedValue();
		Color newSchemeColor = createColorDialog(oldColor);
		if (newSchemeColor != null && !(newSchemeColor.equals(oldColor))) {
	...
	private void replaceColorAction(Color newSchemeColor, Color oldSchemeColor) {
		...
		// add color to current colorScheme
		getCurrentActiveColorScheme().setColor(ColorRGB.convertColorToColorRGB(oldSchemeColor), ColorRGB.convertColorToColorRGB(newSchemeColor));
		...
		for (int i = 0; i < size; i++) {
			Color color = (Color) nominalColorListModel.getElementAt(i);
	...
	private void adaptModels() {
		...
		colorSchemeComboBoxModel.addElement(I18N.getGUILabel("plotter.configuration_dialog.color_scheme_dialog.add_new_scheme.label"));
		colorSchemeComboBox.setSelectedItem(getCurrentActiveColorScheme());
	...
	private void adaptNominalPlot() {
		if (!initializing) {
			// change chart config
			EqualDataFractionGrouping edfg = (EqualDataFractionGrouping) nominalPlotConfig.getDimensionConfig(PlotDimension.COLOR).getGrouping();
...
//			gradientPlotter.updateChartPanelChart(false);
	...
	private void checkIfButtonsEnabled() {
		...
		boolean enableSchemeRemove = !((ColorScheme) colorSchemeComboBox.getSelectedItem()).getName().equals(I18N.getGUILabel("plotter.default_color_scheme_name.label"));
