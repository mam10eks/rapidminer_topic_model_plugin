...
public class DimensionConfigData {
		...
		if (cachedDistinctValues == null) {
			...
			if (getDimensionConfig().isGrouping()) {
				...
				for (Double value : getValues()) {
					// Set.add() returns true if the element was NOT present before the function call
	...
	protected void updateValueCache() {
		...
		boolean useUserDefinedRange = getDimensionConfig().isUsingUserDefinedLowerBound() || getDimensionConfig().isUsingUserDefinedUpperBound();
	...
	public NumericalValueRange getEffectiveRange() {
		...
		if (!getDimensionConfig().isUsingUserDefinedLowerBound() || !getDimensionConfig().isUsingUserDefinedUpperBound()) {
			...
			return (NumericalValueRange) getDimensionConfig().getUserDefinedRangeClone(plotInstance.getPlotData().getOriginalDataTable());
		}
	}
	...
	protected void updateColorProvider() {
		if (getDimensionConfig().isNominal()) {
			...
			setColorProvider(new ContinuousColorProvider(plotInstance, getMinValue(), getMaxValue(), 255, getDimensionConfig().isLogarithmic()));
		}
	}
	...
	protected void updateSizeProvider() {
		...
		if (getDimensionConfig().isNominal()) {
			List<Double> categoryList;
			categoryList = getDistinctValues();
			setSizeProvider(new CategoricalSizeProvider(categoryList, plotConfiguration.getMinShapeSize(), plotConfiguration.getMaxShapeSize()));
		} else {
	...
	public List<PlotConfigurationError> getErrors() {
		...
		if (getDimensionConfig().getDimension() == PlotDimension.SHAPE) {
			...
			if (shapeProvider != null && shapeProvider.maxCategoryCount() < getDistinctValues().size()) {
				PlotConfigurationError error = new PlotConfigurationError("too_many_values_in_dimension", PlotDimension.SHAPE.getName(), getDistinctValues().size(),
						shapeProvider.maxCategoryCount());
				errorList.add(error);
			}
		}
		if (!getDimensionConfig().isNominal() && getDimensionConfig().isLogarithmic() && getMinValue() <= 0.0) {
			PlotConfigurationError error = new PlotConfigurationError("log_axis_contains_zero", getDimensionConfig().getDimension().getName());
			PlotConfigurationQuickFix quickFix = new PlotConfigurationQuickFix(new DimensionConfigChangeEvent(getDimensionConfig(), getDimensionConfig().getDimension(), false,
